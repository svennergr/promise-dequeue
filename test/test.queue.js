...
var queueSrc = require.resolve(
        process.env.PROMISE_QUEUE_COVERAGE ? "../lib-cov" : "../lib"
    ),
...
describe("queue", function() {
    ...
    describe("add", function() {
        ...
        it("rejects if timeout reached", function(done) {
            var queue = new Queue(1, Infinity, { itemTimeout: 5000 });
        ...
        it.only("rejects if timeout reached and calls next item", function(done) {
            var queue = new Queue(1, Infinity, { itemTimeout: 5000 });
            queue
                .add(
                    () =>
